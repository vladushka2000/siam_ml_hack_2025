...
import { AnalyzeProperties } from '../../../../widgets/time-series/types/timeSeriesType';
...
export const TimeSeriesControl = observer(() => {
  ...
  return (
    <Layout direction='row' className='time-series-control'>
      ...
      />
      {
        Object.keys(timeSeriesStore.currentTimeSeriesStore.analyzeProperties).length !== 0 ?
        <>
          <Button
          ...
          > {/* Баг */}
            <Text view='primary' lineHeight='m' size='s'>
              {
                Object.keys(timeSeriesStore.currentTimeSeriesStore.analyzeProperties as AnalyzeProperties).map((key) => {
                  const typedKey = key as keyof typeof timeSeriesStore.currentTimeSeriesStore.analyzeProperties;
                  return (
                    <Text>
                      Параметр {key}: {
                        timeSeriesStore.currentTimeSeriesStore.analyzeProperties[typedKey] ?
                        timeSeriesStore.currentTimeSeriesStore.analyzeProperties[typedKey] :
                        '-'
                      }
                    </Text>
